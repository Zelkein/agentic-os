# CLAUDE.md — CMI (Groupe CMI Experts-Conseils Inc.)

This file keeps Claude Code compatible with the shared `AGENTS.md` guidance and adds Claude-only runtime behavior. **Claude is an optional backend — Kilo is now the primary agent shell.** See `kilo.json` and `.kilo/` for Kilo-native configuration.

**Context:** Frank (Lead Engineer) building MEP coordination automation. Goal: reduce 70H/week → 50H/week.
