...
secondary_types: []
confidence: 0.907
detected_at: 2026-06-08T04:01:32.147Z
signals:
  - kind: manifest
    weight: 0.9
...
|-------|-------------|-------------|
| `quickstart` | "security scan", "vulnerability scan", "find bugs", "security audit", "quickstart" | Front door for the security harness — 30s intro + guided first run |
| `threat-model` | "threat model", "threat modeling", "security review", "attack surface" | Bootstrap/interview threat model → `THREAT_MODEL.md` |
| `vuln-scan` | "vuln scan", "vulnerability scan", "static analysis", "code review security" | Static vulnerability scan → `VULN-FINDINGS.json` |
| `triage` | "triage", "prioritize findings", "sort vulnerabilities" | Verify + dedupe + rank findings pile |
| `patch` | "patch", "fix vulnerability", "generate fix", "security patch" | Generate candidate fixes → `PATCHES/` |
| `customize` | "customize pipeline", "port pipeline", "adapt scanner" | Port the security pipeline to another stack/language |
