...
Keep subtasks high-level — one per major deliverable, not every granular step.`;
  ...
  /**
   * Non-Anthropic provider registry.
   * When a model from one of these providers is selected, Claude Code is
   * redirected through a local translation proxy that converts Anthropic-format
   * requests to OpenAI-compatible API calls.
   */
  private static readonly NON_ANTHROPIC_PROVIDERS: Record<string, {
    target: string;
    apiKeyEnv: string;
    targetModel: string;
  }> = {
    "deepseek-v4-flash": { target: "https://api.deepseek.com/v1", apiKeyEnv: "DEEPSEEK_API_KEY", targetModel: "deepseek-chat" },
    "deepseek-v4-pro":   { target: "https://api.deepseek.com/v1", apiKeyEnv: "DEEPSEEK_API_KEY", targetModel: "deepseek-chat" },
    "deepseek-chat":     { target: "https://api.deepseek.com/v1", apiKeyEnv: "DEEPSEEK_API_KEY", targetModel: "deepseek-chat" },
    "deepseek-coder":    { target: "https://api.deepseek.com/v1", apiKeyEnv: "DEEPSEEK_API_KEY", targetModel: "deepseek-coder" },
    "deepseek-reasoner": { target: "https://api.deepseek.com/v1", apiKeyEnv: "DEEPSEEK_API_KEY", targetModel: "deepseek-reasoner" },
    "glm-4-7-flash":     { target: "https://api.siliconflow.cn/v1", apiKeyEnv: "SILICONFLOW_API_KEY", targetModel: "zai-org/GLM-4.7-Flash" },
  };

  private proxyProcess: ChildProcess | null = null;
  private proxyPort: number | null = null;

  private startProxy(provider: { target: string; apiKeyEnv: string; targetModel: string }): number {
    if (this.proxyProcess && this.proxyPort) return this.proxyPort;

    const apiKey = process.env[provider.apiKeyEnv];
    if (!apiKey) {
      throw new Error(`API key not found for provider: ${provider.apiKeyEnv}`);
    }

    // Find an available port
    const port = 9393;
    console.log(`[process-manager] Starting API proxy on :${port} → ${provider.target}`);
    ...
    const proxyEnv = {
      ...process.env,
      API_PROXY_PORT: String(port),
      API_PROXY_TARGET: provider.target,
      API_PROXY_KEY: apiKey,
      API_PROXY_TARGET_MODEL: provider.targetModel,
    };

    this.proxyProcess = spawnManagedTaskProcess(
      process.execPath,
      [proxyScript],
      {
        env: proxyEnv,
  ...
  private stopProxy(): void {
    if (this.proxyProcess) {
      killChildProcessTree(this.proxyProcess);
  ...
  private spawnClaudeTurn(
    ...
    // Ensure claude.cmd is in PATH on Windows
    if (process.platform === "win32") {
      const claudePath = "C:\\windows\\system32";
      const currentPath = cleanEnv.PATH || "";
      if (!currentPath.includes(claudePath)) {
        cleanEnv.PATH = `${claudePath}${currentPath ? ";" + currentPath : ""}`;
      }
    }
    ...
    // -- Backend routing (BACKEND_MODE override) -----------------
    const backendMode = process.env.BACKEND_MODE || "auto";
    console.log(`[process-manager] Backend mode: ${backendMode}, model: ${model || "(none)"}`);

    let routedModel = model;

    // Resolve via backend-router (passes BACKEND_MODE for kilogateway override)
    const route = backendRouter.resolve(model, backendMode);
    console.log(`[process-manager] Routed to backend: ${route.type}${route.cliModelAlias ? " (" + route.cliModelAlias + ")" : ""}`);

    // -- Kilo gateway execution path -----------------------------
    if (route.type === "kilo-gateway") {
      this.spawnKiloTurn(taskId, prompt, cwd, isContinuation, resumedFromReview, permissionMode, route.cliModelAlias);
      return;
    }

    // -- Non-Anthropic provider routing (api-proxy) --------------
    if (route.type === "api-proxy" && route.proxyConfig) {
      try {
        const port = this.startProxy(route.proxyConfig);
        cleanEnv.ANTHROPIC_BASE_URL = `http://127.0.0.1:${port}/v1`;
        cleanEnv.ANTHROPIC_API_KEY = process.env[route.proxyConfig.apiKeyEnv] || "none";
        routedModel = route.cliModelAlias || "claude-sonnet-4-6";
        console.log(`[process-manager] Proxy active :${port} → ${route.proxyConfig.target} (model=${route.proxyConfig.targetModel})`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[process-manager] Failed to start proxy: ${msg}`);
        this.reportPermissionBridgeFailure(taskId, `Provider proxy unavailable: ${msg}`, msg);
        return;
      }
    }
    // -------------------------------------------------------------
    ...
    try {
      // On Windows, use full path to actual Claude installation; elsewhere use claude
      const claudeCommand = process.platform === "win32"
        ? "C:\\Users\\Frank\\.local\\bin\\claude.cmd"
        : "claude";
      proc = spawnManagedTaskProcess(claudeCommand, args, {
        cwd,
  ...
  private spawnKiloTurn(
    ...
    // Retrieve or create a Kilo session ID for this task.
    // We store it in claudeSessionId (same column, different token format).
    const taskRow = db
      .prepare("SELECT claudeSessionId FROM tasks WHERE id = ?")
      .get(taskId) as { claudeSessionId: string | null } | undefined;
    const kiloSessionId = taskRow?.claudeSessionId ?? null;

    // Build kilo CLI args
    const kiloCommand = process.platform === "win32"
      ? `${process.env.APPDATA || "C:\\Users\\Frank\\AppData\\Roaming"}\\npm\\kilo.cmd`
      : "/home/zelkein/.npm-global/bin/kilo";

    const args: string[] = ["run", "--prompt", prompt];
    if (isContinuation && kiloSessionId) {
      args.push("--session", kiloSessionId);
      console.log(`[process-manager][kilo] Resuming session ${kiloSessionId} for ${taskId.slice(0, 8)}`);
    }

    console.log(`[process-manager][kilo] Spawning kilo${isContinuation ? " (resume)" : ""}: prompt="${prompt.slice(0, 80)}..."`);
    console.log(`[process-manager][kilo] CWD: ${cwd}`);

    let proc;
    try {
      proc = spawnManagedTaskProcess(kiloCommand, args, {
        cwd,
        stdio: ["pipe", "pipe", "pipe"],
        env: { ...process.env },
        detached: true,
      ...
      console.log(`[process-manager][kilo] Spawn succeeded, pid=${proc.pid}`);
    } catch (err) {
      console.error(`[process-manager][kilo] Spawn failed:`, err);
      this.handleSpawnError(taskId, err);
      return;
    }

    // Set up session entry
    const prevSession = this.sessions.get(taskId);
    const session: SessionEntry = {
      proc,
      pendingQuestion: false,
    ...
    // Parse stdout as plain text lines → log entries
    let outputBuffer = "";
    if (proc.stdout) {
      const rl = createInterface({ input: proc.stdout });
      rl.on("line", (line: string) => {
        console.log(`[process-manager][kilo] stdout(${taskId.slice(0, 8)}): ${line.slice(0, 120)}`);
        outputBuffer += (outputBuffer ? "\n" : "") + line;
        // Each non-empty line becomes a text log entry (streamed as it arrives)
        if (line.trim()) {
          this.addLogEntry(taskId, {
            id: crypto.randomUUID(),
            type: "text",
    ...
    if (proc.stderr) {
      proc.stderr.on("data", (chunk: Buffer) => {
        const text = chunk.toString();
        console.log(`[process-manager][kilo] stderr(${taskId.slice(0, 8)}): ${text.trim().slice(0, 200)}`);
        stderrBuffer += text;
      });
    }
    ...
    proc.on("error", (err: Error) => {
      if ((err as NodeJS.ErrnoException).code === "ENOENT") {
        this.handleTaskError(taskId, "Kilo CLI not found. Ensure 'kilo' is installed and in your PATH.");
      } else {
        this.handleTaskError(taskId, `Kilo process error: ${err.message}`);
      }
    });
    ...
    proc.on("close", (code: number | null) => {
      const currentSession = this.sessions.get(taskId);
      if (currentSession && currentSession.proc !== proc) {
        console.log(`[process-manager][kilo] Stale close for ${taskId.slice(0, 8)} — ignoring`);
        return;
      }
      if (!currentSession) {
        console.log(`[process-manager][kilo] Close for ${taskId.slice(0, 8)} with no active session — ignoring`);
        this.lastProgressEmit.delete(taskId);
        return;
      }
      ...
      if (code !== 0) {
        const trimmedStderr = stderrBuffer.trim();
        const errorMsg = trimmedStderr
          ? `Kilo CLI exited with code ${code}: ${trimmedStderr.slice(0, 500)}`
          : `Kilo CLI exited with code ${code}`;
        this.handleTaskError(taskId, errorMsg);
      } else {
        this.handleTurnComplete(taskId, { costUsd: 0, tokensUsed: 0, durationMs: 0 });
      }
