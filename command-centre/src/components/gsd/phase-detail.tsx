  ...
  return (
    ...
    >
      {plan.completed ? (
        ...
        <Circle size={16} style={{ color: "var(--skeleton-bg)", flexShrink: 0 }} />
      )}
      ...
      <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
        {plan.hasPlanFile && (
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "var(--accent-terracotta-light-bg)"; e.currentTarget.style.color = "var(--accent-terracotta-text)"; }}
...
const gsdActions: ReadonlyArray<{
  ...
  { key: "discuss", label: "Discuss", icon: MessageSquare, command: "discuss-phase", color: "var(--text-secondary)", bg: "var(--bg-secondary)", optional: true },
  { key: "plan", label: "Plan", icon: FileEdit, command: "plan-phase", color: "var(--accent-color)", bg: "rgba(147, 69, 42, 0.08)" },
  { key: "execute", label: "Execute", icon: Rocket, command: "execute-phase", color: "var(--info)", bg: "rgba(59, 130, 246, 0.08)" },
  { key: "verify", label: "Verify", icon: ShieldCheck, command: "verify-work", color: "var(--success)", bg: "rgba(107, 142, 107, 0.08)" },
];
...
export function PhaseDetail({ phase, onViewFile, onPhaseUpdated, onCommand }: PhaseDetailProps) {
  ...
  const statusColor = phase.status === "complete" ? "var(--success)" : phase.status === "in-progress" ? "var(--accent-color)" : "var(--text-tertiary)";
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            ...
            <span
              style={{
                ...
                padding: "2px 8px",
                borderRadius: 4,
                backgroundColor: phase.status === "complete" ? "#F0F7F0" : phase.status === "in-progress" ? "#FFF8F5" : "var(--bg-secondary)",
                color: statusColor,
              }}
            >
              {statusLabel}
