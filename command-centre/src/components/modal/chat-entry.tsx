  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 6,
        background: copied ? "rgba(107, 142, 107, 0.12)" : "var(--card-bg)",
...
function CompactRow({
  ...
  const color = accent ?? "var(--text-secondary)";
  return (
    <div
      onClick={onClick}
      style={{
  ...
  const components = {
    ...
    code: ({
      ...
      return cn ? (
        ...
        <code style={{ backgroundColor: "var(--code-bg)", padding: "1px 5px", borderRadius: 3, fontFamily: "var(--font-space-grotesk), Space Grotesk, monospace", fontSize: 13 }}>
          {children}
        </code>
      );
    },
    ...
    >
      <div
        style={{
          ...
          borderLeft: "3px solid var(--border-color-tertiary)",
          ...
          <div style={{ width: "100%", marginTop: 4, marginBottom: 4 }}>
            ...
            >
              ...
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", color: "var(--accent-color)", padding: 0, display: "flex" }}
              >
                <ChevronDown size={12} />
              </button>
            </div>
...
function InlinePreviewPanel({
  ...
  return (
    ...
    >
      {loading ? (
        <span style={{ color: "var(--text-tertiary)", fontSize: 11, padding: isHtml ? 12 : 0 }}>Loading…</span>
      ) : isHtml ? (
        <iframe
        ...
        <span style={{ color: "var(--text-tertiary)", fontSize: 11 }}>Unable to load preview</span>
      )}
    </div>
  );
}
...
export function FileOutputCard({
  ...
  const accentColor = info.op === "create" ? "var(--success-hover)" : "var(--accent-color)";
  const lineBadge =
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        ...
        {previewable && (
          expanded
            ? <ChevronDown size={14} color="var(--text-tertiary)" style={{ flexShrink: 0 }} />
...
function toolVisual(name: string): ToolVisual {
  ...
  if (n === "read") return { Icon: FileText, label: "Read", color: "var(--text-secondary)" };
  if (n === "glob") return { Icon: Search, label: "Search", color: "var(--text-secondary)" };
  if (n === "grep") return { Icon: Search, label: "Search", color: "var(--text-secondary)" };
  if (n === "ls") return { Icon: FolderOpen, label: "List", color: "var(--text-secondary)" };
  if (n === "bash") return { Icon: Terminal, label: "Bash", color: "var(--text-secondary)" };
  if (n === "webfetch") return { Icon: Globe, label: "Fetch", color: "var(--text-secondary)" };
  if (n === "websearch") return { Icon: Globe, label: "Search", color: "var(--text-secondary)" };
  if (n === "todowrite") return { Icon: ListChecks, label: "Updated Todos", color: "var(--text-secondary)" };
  if (n === "task" || n === "agent") return { Icon: Sparkles, label: "Agent", color: "#6D28D9" };
  if (n === "skill") return { Icon: Sparkles, label: "Skill", color: "#6D28D9" };
  return { Icon: Wrench, label: name || "Tool", color: "var(--text-secondary)" };
}
...
function ToolRow({ entry }: { entry: LogEntry }) {
  ...
  return (
    ...
    >
      <visual.Icon size={11} color="var(--text-tertiary)" style={{ flexShrink: 0 }} />
...
function SystemEntry({ entry }: { entry: LogEntry }) {
  return (
    ...
    >
      <Info size={11} color="var(--text-tertiary)" style={{ flexShrink: 0 }} />
