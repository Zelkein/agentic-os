...
function ToolCompactRow({ toolName, entries }: { toolName: string; entries: LogEntry[] }) {
  ...
  if (count === 1) {
    ...
    >
      <Icon size={13} color="var(--text-tertiary)" style={{ flexShrink: 0 }} />
      <span>{label}</span>
      {detail && (
  ...
  return (
    ...
    >
      <Icon size={13} color="var(--text-tertiary)" style={{ flexShrink: 0 }} />
      <span>{label}</span>
      <span style={{ color: "var(--text-tertiary)" }}>· {count} {noun}</span>
    </div>
  );
}
...
function TodoUpdateCard({ entry }: { entry: LogEntry }) {
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <ListChecks size={13} color="var(--text-tertiary)" style={{ flexShrink: 0 }} />
            ...
            return (
              <div
                ...
                style={{
                  ...
                  color: isDone ? "var(--success)" : isActive ? "var(--accent-color)" : "var(--text-secondary)",
                  lineHeight: 1.4,
                }}
              >
                {isDone ? (
                  ...
                  <Circle size={12} color={isActive ? "var(--accent-color)" : "var(--text-tertiary)"} style={{ flexShrink: 0 }} />
                )}
      ...
      <button
        ...
        style={{
          ...
          background: message.trim() && !isSending
            ...
            : "var(--skeleton-bg)",
          color: message.trim() && !isSending ? "var(--card-bg)" : "var(--text-secondary)",
        ...
        {renderItems.map((item, index) => {
          ...
          if (item.kind === "phase-divider") {
            ...
            return (
              ...
              >
                <div style={{ flex: 1, height: 1, backgroundColor: "var(--skeleton-bg)" }} />
                <span
                  style={{
                    fontSize: 11,
                    fontFamily: "var(--font-space-grotesk), Space Grotesk, sans-serif",
          ...
          if (item.kind === "child-review") {
            return (
              ...
              >
                ...
                >
                  <strong style={{ color: "var(--accent-color)" }}>{item.childTask.title}</strong> is ready for review
                </span>
              </div>
            );
          }
