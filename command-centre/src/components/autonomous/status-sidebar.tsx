...
function TaskRow({ task, onClick }: { task: Task; onClick: (id: string) => void }) {
  const statusIcon = {
    running: <Loader size={12} style={{ color: "var(--accent-color)", animation: "spin 2s linear infinite" }} />,
    queued: <Play size={12} style={{ color: "var(--text-secondary)" }} />,
    review: <HelpCircle size={12} style={{ color: "#D4A574" }} />,
    done: <CheckCircle size={12} style={{ color: "var(--success)" }} />,
    backlog: <Play size={12} style={{ color: "var(--text-tertiary)" }} />,
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
        <StatusGroup label="Running" tasks={running} color="var(--accent-color)" onTaskClick={openPanel} />
        <StatusGroup label="Needs Your Input" tasks={needsInput} color="#D4A574" onTaskClick={openPanel} />
        <StatusGroup label="History" tasks={done} color="var(--success)" onTaskClick={openPanel} />
