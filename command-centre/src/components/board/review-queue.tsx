...
export function ReviewQueue({ tasks, allTasks }: { tasks: Task[]; allTasks?: Task[] }) {
  ...
  const accentColor = isError ? "var(--error)" : isInput ? "var(--warning)" : "var(--accent-color)";
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 12px", borderBottom: "1px solid rgba(218, 193, 185, 0.12)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {queue.length > 1 && (
            <span style={{ fontSize: 10, fontFamily: "var(--font-space-grotesk), Space Grotesk, sans-serif", color: "var(--text-tertiary)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          }} label="Done" hover="var(--success)"><CheckCircle2 size={14} /></Btn>
        </div>
      </div>
      ...
      {projectContext && (
        <div style={{
          ...
          <span style={{ color: "var(--text-secondary)", fontWeight: 500 }}>{projectContext.name}</span>
          <span>&middot;</span>
      ...
      <div style={{ padding: "12px 14px 8px" }}>
        ...
        {prompt && (
          <div style={{
            ...
            {isError && <div style={{ ...LABEL, color: "var(--error)", marginBottom: 2 }}>Error</div>}
            <div style={{ ...BODY_TEXT, fontSize: 13, color: isError ? "#8B3A2E" : "var(--text-secondary)" }}>
              {prompt}
            </div>
          </div>
        )}
      ...
      <div style={{ padding: "6px 14px 10px", borderTop: "1px solid rgba(218, 193, 185, 0.08)" }}>
        ...
        {justSent ? (
          <div style={{ fontSize: 12, fontFamily: "var(--font-inter), Inter, sans-serif", color: "var(--success)", padding: "5px 0", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <>
            {(attachedFile || pastedChips.length > 0) && (
              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 8 }}>
                ...
                {attachedFile && (
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 7px", backgroundColor: "rgba(147, 69, 42, 0.06)", borderRadius: 4, fontSize: 11, fontFamily: "var(--font-space-grotesk), Space Grotesk, sans-serif", color: "var(--accent-color)" }}>
                    <Paperclip size={9} /> {attachedFile.name}
                    <button onClick={() => setAttachedFile(null)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: "var(--text-tertiary)", display: "flex" }}><X size={9} /></button>
                  </div>
                )}
                {pastedChips.filter((chip) => chip.kind === "image").map((chip) => (
                  <div key={chip.id} style={{
                    ...
                    color: chip.kind === "image" ? "var(--accent-color)" : "#3b6ec2",
                  }}>
                    {chip.kind === "image" ? <span style={{ fontSize: 10 }}>&#128247;</span> : <FileText size={9} />}
                    <span>Pasted {chip.label}</span>
                    <button onClick={() => setPastedChips((prev) => removePendingPastedText(prev, chip.id))} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: "var(--text-tertiary)", display: "flex" }}><X size={9} /></button>
                  </div>
                ))}
                </div>
              </div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <textarea
                ...
                style={{
                  ...
                  padding: "8px 62px 8px 12px", backgroundColor: "var(--card-bg)",
                  outline: "1px solid rgba(218, 193, 185, 0.2)", borderRadius: 8,
                  border: "none", resize: "none", lineHeight: 1.5, color: "var(--text-primary)", boxSizing: "border-box",
              ...
              <div style={{ position: "absolute", right: 4, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: 2 }}>
                <button onClick={() => fileInputRef.current?.click()} disabled={uploading} title="Attach file"
                  style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 6, border: "none", background: "none", cursor: uploading ? "wait" : "pointer", color: attachedFile ? "var(--accent-color)" : "#B0B0B5", transition: "color 100ms ease" }}
                  onMouseEnter={(e) => { e.currentTarget.style.color = "var(--accent-color)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = attachedFile ? "var(--accent-color)" : "#B0B0B5"; }}
                >{uploading ? <span style={{ width: 11, height: 11, borderRadius: "50%", border: "2px solid var(--border-color)", borderTopColor: "var(--accent-color)", animation: "spin 1s linear infinite", display: "inline-block" }} /> : <Paperclip size={14} />}</button>
                <button onClick={handleSubmit} disabled={(!message.trim() && !attachedFile && pastedChips.length === 0) || isSending}
                  style={{
                    ...
                    color: (message.trim() || attachedFile || pastedChips.length > 0) && !isSending ? "var(--card-bg)" : "#C0C0C4",
...
function Btn({ onClick, label, children, hover }: { onClick: () => void; label: string; children: React.ReactNode; hover?: string }) {
  return (
    <button onClick={(e) => { e.stopPropagation(); onClick(); }} title={label}
      style={{ display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "none", cursor: "pointer", padding: 4, borderRadius: 4, color: "var(--text-tertiary)", transition: "color 100ms ease" }}
      onMouseEnter={(e) => { e.currentTarget.style.color = hover || "var(--text-secondary)"; }} onMouseLeave={(e) => { e.currentTarget.style.color = "var(--text-tertiary)"; }}
    >{children}</button>
  );
}
