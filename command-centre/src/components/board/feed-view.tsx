...
const mdComponents = {
  ...
  a: ({ href, children }: { href?: string; children?: React.ReactNode }) => (
    <a href={href} style={{ color: "var(--accent-color)", textDecoration: "underline" }} target="_blank" rel="noopener noreferrer">{children}</a>
  ),
  ...
  blockquote: ({ children }: { children?: React.ReactNode }) => <blockquote style={{ borderLeft: "3px solid #93452A", paddingLeft: 12, margin: "6px 0", color: "var(--text-secondary)", fontStyle: "italic" }}>{children}</blockquote>,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              ...
              {showTagEditor ? (
                ...
                <TagPill tag={typeof task.tag === 'string' ? { id: task.tag, name: task.tag, color: '#6366f1', clientId: null, createdAt: new Date().toISOString() } : task.tag} />
              ) : null}
...
function DoneDropZone({
  ...
  return (
    ...
    >
      <CheckCircle2 size={16} color={isOver ? "var(--success)" : "#aaa"} />
      <span style={{
        fontSize: 13,
        fontWeight: 500,
        color: isOver ? "var(--success)" : "#aaa",
      ...
      {visible.map((st) => {
        ...
        const visuals: Record<DisplayState, {
          ...
          statusLabel: string; statusColor: string; strike: boolean;
            ...
            textColor: "var(--text-tertiary)", dotColor: "var(--success)", pulse: false,
            statusLabel: "done", statusColor: "var(--text-tertiary)", strike: true,
          },
          needs: {
            bg: "rgba(232, 149, 109, 0.06)", bgHover: "rgba(232, 149, 109, 0.12)",
            border: "1px solid rgba(232, 149, 109, 0.35)", borderLeft: "3px solid #e8956d",
            textColor: "var(--accent-color)", dotColor: "var(--accent-color)", pulse: false,
            statusLabel: "input", statusColor: "var(--accent-color)", strike: false,
          },
          running: {
            bg: "transparent", bgHover: "rgba(0,0,0,0.03)",
            border: "1px solid transparent", borderLeft: "3px solid rgba(147, 69, 42, 0.5)",
            textColor: "var(--text-secondary)", dotColor: "var(--card-bg)", pulse: true,
            statusLabel: "running", statusColor: "var(--text-tertiary)", strike: false,
          },
          queued: {
            bg: "transparent", bgHover: "rgba(0,0,0,0.03)",
            border: "1px solid transparent", borderLeft: "3px solid transparent",
            textColor: "var(--text-secondary)", dotColor: "#ccc", pulse: false,
            statusLabel: "queued", statusColor: "var(--text-tertiary)", strike: false,
          },
          backlog: {
            bg: "transparent", bgHover: "rgba(0,0,0,0.03)",
            border: "1px solid transparent", borderLeft: "3px solid transparent",
            textColor: "var(--text-secondary)", dotColor: "var(--border-color)", pulse: false,
            statusLabel: st.status, statusColor: "var(--text-tertiary)", strike: false,
          },
        };
      ...
      {hiddenCount > 0 && !expanded && (
        <button
          ...
          style={{
            ...
            fontSize: 10, fontFamily: "'DM Mono', monospace", color: "var(--text-tertiary)",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: "var(--text-tertiary)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          {!loading && content !== null && !editing && (
            <button
              ...
              style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-tertiary)", display: "flex", padding: 2, fontSize: 10, fontFamily: "'DM Mono', monospace" }}
              title="Edit"
            >
              Edit
            </button>
          ...
          {editing && (
            <>
              ...
              <button
                ...
                style={{
                  background: draft !== content ? "var(--accent-color)" : "#ccc", border: "none", cursor: draft !== content ? "pointer" : "default",
                  color: "var(--text-primary)", display: "flex", padding: "2px 8px", fontSize: 10, fontFamily: "'DM Mono', monospace", borderRadius: 3,
                }}
              >
                {saving ? "Saving..." : "Save"}
              </button>
      ...
      <div style={{
        ...
        {loading ? (
          <span style={{ padding: 12, color: "var(--text-tertiary)" }}>Loading...</span>
        ) : editing ? (
          <textarea
            value={draft}
          ...
          <span style={{ color: "var(--text-tertiary)" }}>File not found</span>
        )}
      </div>
    </div>
  );
...
function GsdPhasesOverview({
  ...
  if (phases.length === 0) {
    return (
      <div style={{ fontSize: 13, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace", padding: "12px 0" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div>
          <button
            ...
            style={{
              ...
              fontSize: 11, fontFamily: "'DM Mono', monospace", color: "var(--accent-color)",
              ...
              {projectFiles.map((f) => (
                <button
                  ...
                  style={{
                    ...
                    padding: "4px 8px", fontSize: 11, color: "var(--text-secondary)", textAlign: "left",
                ...
                >
                  <FileText size={10} color="var(--text-tertiary)" />
                  {f.name}
                </button>
              ))}
              {researchFiles.length > 0 && (
                <>
                  <div style={{ fontSize: 9, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace", padding: "4px 8px", marginTop: 4 }}>
                    Research
                  </div>
                  {researchFiles.map((f) => (
                    <button
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{
          flex: 1, height: 4, borderRadius: 2, background: "var(--skeleton-bg)", overflow: "hidden",
        }}>
          <div style={{
        ...
        <span style={{
          fontSize: 10, fontFamily: "'DM Mono', monospace", color: "var(--text-tertiary)", flexShrink: 0,
        }}>
      ...
      {phases.map((phase) => {
        ...
        return (
          <div key={phase.phaseNumber} style={{
            padding: "8px 10px",
            borderRadius: 6,
            background: allDone ? "rgba(122, 184, 122, 0.04)" : inProgress ? "var(--pane-bg)" : "var(--pane-bg)",
            border: isExpanded
          ...
          >
            ...
            <div style={{
              ...
              <span style={{
                ...
                color: allDone ? "var(--success)" : inProgress ? "var(--accent-color)" : "#bbb",
                minWidth: 14,
              }}>
                {phase.phaseNumber}
              </span>
              ...
              {allDone && <CheckCircle2 size={12} color="var(--success)" />}
              <span style={{
                fontSize: 8, display: "inline-block",
                transition: "transform 150ms ease",
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        background: "transparent",
                        cursor: "pointer",
                        color: isDone ? "var(--success-hover)"
            ...
            {currentStep && (
              <div style={{
                ...
                color: taskNeedsInput(currentStep) ? "var(--accent-color)" : "rgba(147, 69, 42, 0.6)",
            ...
            {isExpanded && (
              ...
              >
                {loadingFiles ? (
                  <span style={{ fontSize: 11, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace" }}>Loading...</span>
                ) : phaseFiles.length === 0 ? (
                  <span style={{ fontSize: 11, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace" }}>No documents for this phase</span>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    {phaseFiles.map((f) => (
                      <button
                        ...
                        style={{
                          ...
                          background: viewingDoc === f.relativePath ? "rgba(147, 69, 42, 0.06)" : "transparent",
                          border: "none", borderRadius: 4, cursor: "pointer",
                          padding: "4px 6px", fontSize: 11, color: "var(--text-secondary)", textAlign: "left",
                          fontFamily: "'DM Mono', monospace", transition: "background 100ms ease",
                        }}
                        onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(0,0,0,0.03)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.background = viewingDoc === f.relativePath ? "rgba(147, 69, 42, 0.06)" : "transparent"; }}
    ...
    <div style={{
                ...
                return (
                  <span key={segPath} style={{ display: "inline-flex", alignItems: "center", gap: 2 }}>
                    <button
                      ...
                      onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.textDecorationColor = "var(--accent-color)"; }}
          ...
          {editing && (
            <>
              <button
                onClick={() => setEditing(false)}
                title="Cancel"
                style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-tertiary)", fontSize: 10, fontFamily: "'DM Mono', monospace", padding: "2px 6px" }}
              >
                Cancel
              </button>
              <button
      ...
      <div style={{
        ...
        {loading ? (
                    ...
                    <li style={{ margin: "2px 0", listStyle: "none", marginLeft: -20 }}>
                      <label style={{ display: "flex", alignItems: "flex-start", gap: 6, cursor: "pointer" }}>
                        <input
                          ...
                          style={{ marginTop: 3, cursor: "pointer", accentColor: "var(--accent-color)" }}
                        />
          ...
          <span style={{ color: "var(--text-tertiary)" }}>Preview unavailable</span>
        )}
      </div>
      )}
    </div>
    ...
    >
      ...
      {!isFullscreen && onStartDrawerDrag && (
        <div
          ...
          onMouseEnter={(e) => {
            (e.currentTarget.firstChild as HTMLElement | null)?.style?.setProperty("background", "var(--accent-color)");
          }}
          onMouseLeave={(e) => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          {parentOfCurrent && onNavigateToParent ? (
            ...
            <InlineTitle title={task.title} taskId={task.id} color="var(--text-primary)" />
          )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {onToggleFullscreen && (
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.color = "var(--card-bg)"; }}
      ...
      {!pendingScope && (<div style={{
            ...
            <div key={t.key} style={{ display: "flex", alignItems: "center" }}>
              <button
                ...
                style={{
                  ...
                  color: mainTab === t.key || (t.key === "plan" && planDocked) ? "var(--card-bg)" : "#bbb",
                  background: "transparent",
                  border: "none",
              ...
              {t.key === "plan" && (
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    cursor: "pointer",
                    color: planDocked ? "var(--accent-color)" : "#bbb",
                ...
                <div style={{ position: "relative", minWidth: 0, overflow: "hidden" }}>
                  ...
                  <textarea
                    ...
                    style={{
                      ...
                      border: "none",
                      color: (replyText.includes("@") || replyText.includes("/")) ? "transparent" : "var(--card-bg)",
                      caretColor: "var(--card-bg)",
                      outline: "none",
                      resize: "none",
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    color: replyAttachments.length > 0 ? "var(--accent-color)" : "var(--text-secondary)",
        ...
        {mainTab === "plan" && task.projectSlug && (
          <div style={{ flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
            ...
            {!planDocked && (
              <div style={{ padding: "8px 12px", borderTop: "1px solid #e8e4df", background: "var(--dropdown-bg)", flexShrink: 0 }}>
                <button
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {task.projectSlug && (
                <div style={{ marginBottom: 4 }}>
                  <div style={{ fontSize: 9, fontFamily: "'DM Mono', monospace", color: "var(--text-tertiary)", letterSpacing: "0.06em", marginBottom: 4, textTransform: "uppercase" }}>
                    Plan
                  </div>
                  <button
                    onClick={() => {
                  ...
                  >
                    <FileText size={10} color="var(--accent-color)" />
                    brief.md
                  </button>
                </div>
              )}
              ...
              {task.level === "gsd" && planningFiles && (
                <>
                  ...
                  {outputFiles.length > 0 && (
                    <div style={{ height: 1, background: "var(--skeleton-bg)", margin: "4px 0" }} />
                  )}
                </>
              )}
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 4 }}>
                    <button
                      ...
                      style={{
                        ...
                        color: changedOnly ? "var(--accent-color)" : "#999",
                        cursor: "pointer",
                  ...
                  return (
                    <div key={file.id}>
                      <div style={{
                        padding: "10px 12px",
                        borderRadius: 8,
                        backgroundColor: isActive ? "rgba(147, 69, 42, 0.05)" : "var(--pane-bg)",
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{
                            width: 28, height: 28, borderRadius: 5, backgroundColor: "var(--skeleton-bg)",
                            display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                          }}>
                            <span style={{ fontSize: 8, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                              <span style={{
                                fontSize: 12, fontWeight: 500, color: "var(--card-bg)", overflow: "hidden",
                            ...
                            {breadcrumb && (
                              <div style={{
                                fontSize: 10, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace",
                        ...
                        <div style={{
                          ...
                          {canPreview && (
                            <button
                              ...
                              style={{
                                display: "flex", alignItems: "center", gap: 4,
                                background: "none", border: "none", cursor: "pointer",
                                color: isActive ? "var(--accent-color)" : "#999", fontSize: 10,
                                fontFamily: "'DM Mono', monospace", padding: "3px 6px", borderRadius: 4,
                                transition: "color 0.15s, background 0.15s",
                              }}
                              onMouseEnter={(e) => { e.currentTarget.style.color = "var(--accent-color)"; e.currentTarget.style.background = "rgba(147, 69, 42, 0.06)"; }}
                          ...
                          <a
                            ...
                            style={{
                              display: "flex", alignItems: "center", gap: 4,
                              color: "var(--text-tertiary)", fontSize: 10, textDecoration: "none",
                              fontFamily: "'DM Mono', monospace", padding: "3px 6px", borderRadius: 4,
                              transition: "color 0.15s, background 0.15s",
                            }}
                            onMouseEnter={(e) => { e.currentTarget.style.color = "var(--accent-color)"; e.currentTarget.style.background = "rgba(147, 69, 42, 0.06)"; }}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 6,
        border: isSelected
          ? `1px solid ${task.level === "gsd" ? "#6D28D9" : task.level === "project" ? "var(--accent-color)" : "rgba(147, 69, 42, 0.35)"}`
          : `1px dashed ${borderColor}`,
        background: isSelected ? "var(--card-bg)" : "var(--pane-bg)",
        cursor: "pointer",
        fontSize: 12,
        color: "var(--text-secondary)",
      ...
      onMouseLeave={(e) => { if (!isSelected) { e.currentTarget.style.background = "var(--pane-bg)"; e.currentTarget.style.opacity = "0.85"; } }}
    >
        ...
        return (
          ...
          >
            <span style={{
              ...
              height: 6,
              borderRadius: "50%",
              background: isRunning ? "var(--card-bg)" : "#ccc",
              flexShrink: 0,
              marginTop: 4,
              alignSelf: "flex-start",
...
function RecentOutputsSection({
  ...
  return (
    <div data-card style={{
      ...
      {outputs.length === 0 ? (
        <div style={{ fontSize: 11, color: "var(--text-tertiary)", fontFamily: "'DM Mono', monospace", padding: "4px 0" }}>
          No outputs yet
        </div>
      ) : (
              ...
              <div key={o.id}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{
                      fontSize: 9, fontFamily: "'DM Mono', monospace", color: "var(--text-tertiary)",
                  ...
                  <span style={{
                    ...
                    background: "var(--dropdown-hover)", color: "var(--text-tertiary)", flexShrink: 0,
                  }}>
                    .{o.extension}
                  </span>
                </div>
...
const [doneFilter, setDoneFilter] = useState<DoneFilter>("1d");
  ...
  const [boardColumns, setBoardColumns] = useState<any[] | null>(null);
  const [showBoardSettings, setShowBoardSettings] = useState(false);

  // Fetch board columns config
  useEffect(() => {
    fetch("/api/board-columns")
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) setBoardColumns(data);
      })
      .catch(() => {});
  }, []);
  ...
  const bucketedLanes = useMemo(() => {
    ...
    for (const [slug, entry] of byClient) {
      ...
      lanes.push({
        ...
        clientColor: client?.color || "var(--text-tertiary)",
  ...
  const handleDropColumn = useCallback((column: string, e: React.DragEvent) => {
    e.preventDefault();
  ...
  const handleDragOverColumn = useCallback((_column: string, e: React.DragEvent) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = "move";
  ...
  const handleDragLeaveColumn = useCallback((_column: string) => {
...
<TagFilterBar
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", padding: "4px 12px" }}>
        <button
          ...
          onMouseLeave={(e) => { e.currentTarget.style.color = "var(--text-secondary)"; e.currentTarget.style.borderColor = "var(--border-color-secondary)"; }}
        >
          Board Settings
        </button>
      </div>
...
groupByTag={groupByTag}
        ...
        onToggleGroupByPriority={() => setGroupByPriority((v) => !v)}
        columns={boardColumns ?? undefined}
  ...
  return (
    <button
      ...
      style={{
        ...
        color: copied ? "var(--success)" : "var(--accent-color)",
        cursor: "pointer",
        transition: "all 150ms ease",
        whiteSpace: "nowrap",
        flexShrink: 0,
