...
export function RuntimeStatus() {
  ...
  const accentColor = isHealthyLeader ? "#10B981" : "var(--accent-terracotta-dark)";
  ...
  return (
    ...
    >
      <div style={{ minWidth: 0 }}>
        ...
        >
          Reason: <span style={{ color: "var(--text-primary)" }}>{systemStatus.ownershipReason}</span>
        </div>
      </div>

      <div>
        <div style={{ fontSize: 10, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.12em" }}>
          Runtime
        </div>
        <div style={{ fontSize: 13, color: "var(--text-primary)", marginTop: 6 }}>{systemStatus.runtime}</div>
