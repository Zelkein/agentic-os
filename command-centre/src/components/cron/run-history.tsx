...
function getRunResultBadge(run: CronRun) {
  ...
  switch (run.result) {
    ...
    case "failure":
      return { label: "Failed", backgroundColor: "var(--error-bg)", color: "var(--error)" };
    case "timeout":
      return { label: "Timeout", backgroundColor: "#FFF7ED", color: "#EA580C" };
    default:
      return { label: "Running", backgroundColor: "var(--info-bg)", color: "var(--info)" };
  }
}
...
function OutputLink({ output }: { output: CronRunOutput }) {
  ...
  return (
    <a
      ...
      onMouseLeave={(e) => {
        e.currentTarget.style.backgroundColor = "var(--accent-terracotta-light-bg)";
      }}
    >
      <Icon size={10} />
