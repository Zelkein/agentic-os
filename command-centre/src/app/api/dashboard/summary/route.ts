...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check root USER.md first (human format), then context/USER.md (template)
    const rootUserPath = path.join(baseDir, "USER.md");
    const contextUserPath = path.join(baseDir, "context", "USER.md");

    for (const filePath of [rootUserPath, contextUserPath]) {
      if (userName) break;
      if (!fs.existsSync(filePath)) continue;
      const userContent = fs.readFileSync(filePath, "utf-8");

      // Pattern 1: "# Frank (Francis B. Morissette)" — take first name
      const hashMatch = userContent.match(/^#\s+(\w+)/m);
      if (hashMatch && hashMatch[1] && !/^user$/i.test(hashMatch[1])) {
        userName = hashMatch[1];
        break;
      }

      // Pattern 2: "- Name: Francis B. Morissette" (horizontal whitespace only)
      const dashMatch = userContent.match(/^- Name:[ \t]*(.+)/m);
      if (dashMatch && dashMatch[1].trim() && dashMatch[1].trim() !== "-") {
        userName = dashMatch[1].trim();
        break;
      }
    }
    ...
    ).all(...clientParams) as Array<{ status: string; count: number }>;
    ...
    const priorityDist = db.prepare(
      `SELECT priority, COUNT(*) as count FROM tasks
       ...
       GROUP BY priority ORDER BY
         ...
         END`
    ).all(...clientParams) as Array<{ priority: string; count: number }>;

    // -- Recent notifications ----------------------------------------
    const recentNotifications = db.prepare(
      `SELECT id, type, title, message, taskId, read, createdAt
       FROM notifications
       ORDER BY createdAt DESC LIMIT 5`
    ).all() as Array<{
      id: string; type: string; title: string;
      message: string | null; taskId: string | null;
